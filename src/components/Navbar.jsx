...
const Navbar = () => {
    ...
    return (
        <div id="navbar-scroll" className="">
            {/* Navbar */}
            <div className={`fixed z-50 top-0 left-0 right-0 transition-all duration-300 ${scrolled 
                ...
                <div className="max-w-7xl mx-auto px-4 md:px-6 flex items-center justify-between">
                    ...
                    <div>
                         <a href="/" className="text-2xl font-bold tracking-tighter text-black dark:text-white">
                            RF<span className="text-gray-500">.</span>
                         </a>
                    </div>

                    {/* Desktop Menu */}
                    <div className="hidden md:flex items-center space-x-8">
                        <ul className="flex space-x-6 text-base font-medium text-gray-700 dark:text-gray-300">
                            {navLinks.map((link) => (
                                <li key={link.name}>
                                    <ScrollLink
                                        to={link.to}
                    ...
                    <div className="md:hidden flex items-center gap-4">
                        ...
                        <button onClick={toggleMenu} className="text-2xl text-black dark:text-white focus:outline-none" aria-label="Toggle menu">
                            {menuOpen ? "✕" : "☰"}
                        </button>
                    </div>
                </div>
                ...
                {/* Mobile Menu */}
                <div
                    className={`md:hidden absolute top-full left-0 w-full bg-white dark:bg-background shadow-lg overflow-hidden transition-all duration-300 ease-in-out ${menuOpen ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"}`}
                >
